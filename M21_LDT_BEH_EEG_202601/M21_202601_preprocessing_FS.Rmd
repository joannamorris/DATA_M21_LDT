...
```{r}
erp_dmg_words <- erp_dmg |> filter(bini %in% c(17:18)) |>
  select(SubjID, value,chindex,chlabel,bini,binlabel,List,time_window,z_vcb,z_spl,z_art,Dim.1,Dim.2)
erp_dmg_nonwords <- erp_dmg |> filter(bini %in% c(19:22)) |>
  select(SubjID, value,chindex,chlabel,bini,binlabel,List,time_window,z_vcb,z_spl,z_art,Dim.1,Dim.2)

# Words
erp_dmg_words <- erp_dmg_words |>
  separate(binlabel, into = c("trial_type","base_frequency"), sep = "_", remove = TRUE) |>
  select(-trial_type)

# Nonwords
erp_dmg_nonwords <- erp_dmg_nonwords |>
  separate(binlabel, into = c("trial_type", "complexity", "base_frequency"), sep = "_", remove = TRUE) |>
  select(-trial_type)
...
erp_dmg_nonwords <- erp_dmg_nonwords |>
  mutate(base_frequency = fct_relevel(family_size, "High", "Low"))


write_csv(erp_dmg_words, "erp_bf_dmg_words.csv")
write_csv(erp_dmg_nonwords, "erp_bf_dmg_nonwords.csv")

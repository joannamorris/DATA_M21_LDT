...
This script computes one ANOVA for all non-words,
...
nw <- nw |> mutate(rt.err.imp = ifelse(is.na(rt.err), 
                                       nw.mean, 
                                       rt.err))  # replace missing values with mean
...
p2 <-  nw.cond.means %>% ggplot(aes(x=lang_type, 
                                    y=mean, 
                                    fill = nw_famsize, 
                                    ymin = mean - se, 
                                    ymax = mean + se)) +
  coord_cartesian(xlim = NULL, ylim = c(500, 750), 
                  expand = TRUE, 
                  default = FALSE,
                  clip = "on") +
...
# Main effects models with random intercepts
nw_main.model = lmer(rt.err.imp ~ lang_type + nw_famsize + complexity +
                       (1|SubjID) + 
                       (1|nw_target),
                     data= nw, 
                     REML=FALSE)

...
Export list of subjects with Reading type classification

```{r}
df <- distinct(select(cw, SubjID, lang_type ))
df_o <- filter(df, lang_type == "Orthographic")
df_s <- filter(df, lang_type != "Orthographic")
write_csv(df, "m21_subjlist_reading_type.csv")
write_csv(df_o, "m21_subjlist_ortho.csv")
write_csv(df_s, "m21_subjlist_semant.csv")
```
...
library(ez)
library(car)
(m.cw <- ezANOVA(cw,dv = rt.err.imp,wid = SubjID,within = cw_famsize,between = lang_type))

(m.nw_smpl <- ezANOVA(nw_smpl,dv = rt.err.imp,wid = SubjID,within = .(nw_famsize),between = lang_type))

(m.nw_cplx <- ezANOVA(nw_cplx,dv = rt.err.imp,wid = SubjID,within = .(nw_famsize),between = lang_type))

m1 <- aov(rt.err.imp ~ lang_type * cw_famsize, data = cw)
m2 <- aov(rt.err.imp ~ lang_type * nw_famsize, data = nw_smpl)
m3 <- aov(rt.err.imp ~ lang_type * nw_famsize, data = nw_cplx)
summary(m1)
summary(m2)
summary(m3)
```

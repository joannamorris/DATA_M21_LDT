...
Loads in original Rt file, removes duplicates, puts target in lowercase, and removes rows and columns with only missign values. `duplicated()` is an R base function that takes vector or data.frame as input and selects rows that are duplicates, by negating the result you will remove all duplicate rows in the R data.frame.

```{r}
df <- read_csv("m21_rt/Aggregated_rt_data/ALL_rt_allvar.csv")
df <- df[!duplicated(df), ]
df <- mutate(df, target_lower = tolower(target))
df$target <- NULL
df[1] <- NULL
...
write_csv(df,"m21_rt/Aggregated_rt_data/ALL_rt_allvar.csv" )
```

---
title: "M21 LDT ERP HC SEMANTIC SENSITIVITY"
...
|----------------------------------|--------|----------|--------|---------|
|                       family_size| 1, 2121|   8.95 **|    .003|4.20e-03 | 
|                         base_freq| 1, 2121|   5.88  *|    .015|2.76e-03 |
|             family_size:base_freq| 1, 2121| 14.30 ***|   <.001|6.70e-03 |
| Sensitivity:family_size:base_freq| 1, 2121|  4.90 *  |    .027|2.30e-03 |
...
```{r}
# Semantic Sensitivity x Family Size x Base Frequency

selected_contrasts_hisem_famsize <- c("High Semantic Small Family High Base Frequency - High Semantic Small Family Low Base Frequency",
                                       "High Semantic Large Family High Base Frequency - High Semantic Large Family Low Base Frequency")
selected_contrasts_losem_famsize <- c("Low Semantic Small Family High Base Frequency - Low Semantic Small Family Low Base Frequency",
                                       "Low Semantic Large Family High Base Frequency - Low Semantic Large Family Low Base Frequency")
selected_contrasts_hisem_basefrq <- c("High Semantic Large Family High Base Frequency - High Semantic Small Family High Base Frequency",
                                       "High Semantic Large Family Low Base Frequency - High Semantic Small Family Low Base Frequency")
selected_contrasts_losem_basefrq <- c("Low Semantic Large Family High Base Frequency - Low Semantic Small Family High Base Frequency",
                                      "Low Semantic Large Family Low Base Frequency - Low Semantic Small Family Low Base Frequency")      

emmeans_obj <- emmeans(anova_model_1a,  pairwise ~ Semantic_Sensitivity * family_size * base_freq, adjust = "bonferroni",  pbkrtest.limit = 6480)

contrasts_df <- as.data.frame(emmeans_obj$contrasts)
selected_contrasts_hisem_famsize_df <- as.data.frame(emmeans_obj$contrasts[contrasts_df$contrast %in% selected_contrasts_hisem_famsize, ])
selected_contrasts_losem_famsize_df <- as.data.frame(emmeans_obj$contrasts[contrasts_df$contrast %in% selected_contrasts_losem_famsize, ])
selected_contrasts_hisem_basefrq_df <- as.data.frame(emmeans_obj$contrasts[contrasts_df$contrast %in% selected_contrasts_hisem_basefrq, ])
selected_contrasts_losem_basefrq_df <- as.data.frame(emmeans_obj$contrasts[contrasts_df$contrast %in% selected_contrasts_losem_basefrq, ])

cohensd_1 <- as.data.frame(cohens_d(value ~ base_freq, data = subset(n250_words_b, Semantic_Sensitivity == "High Semantic" & family_size == "Large Family")))
cohensd_2 <- as.data.frame(cohens_d(value ~ base_freq, data = subset(n250_words_b, Semantic_Sensitivity == "High Semantic" & family_size == "Small Family")))

cohensd_3 <- as.data.frame(cohens_d(value ~ base_freq,  data = subset(n250_words_b, Semantic_Sensitivity == "Low Semantic" & family_size == "Large Family")))
cohensd_4 <- as.data.frame(cohens_d(value ~ base_freq, data = subset(n250_words_b, Semantic_Sensitivity == "Low Semantic" & family_size == "Small Family")))

cohensd_5 <- as.data.frame(cohens_d(value ~ family_size,  data = subset(n250_words_b, Semantic_Sensitivity == "High Semantic" & base_freq == "High Base Frequency")))
cohensd_6 <- as.data.frame(cohens_d(value ~ family_size,  data = subset(n250_words_b, Semantic_Sensitivity == "High Semantic" & base_freq == "Low Base Frequency")))

cohensd_7 <- as.data.frame(cohens_d(value ~ family_size,  data = subset(n250_words_b, Semantic_Sensitivity == "Low Semantic" & base_freq == "High Base Frequency")))
cohensd_8 <- as.data.frame(cohens_d(value ~ family_size, data = subset(n250_words_b, Semantic_Sensitivity == "Low Semantic" & base_freq == "Low Base Frequency")))

cohensd_hisem_famsize_df<- bind_rows(cohensd_1, cohensd_2)
cohensd_losem_famsize_df <- bind_rows(cohensd_3, cohensd_4)
cohensd_hisem_basefrq_df <- bind_rows(cohensd_5, cohensd_6)
cohensd_losem_basefrq_df <- bind_rows(cohensd_7, cohensd_8)


(hisem_famsize_df <- bind_cols(selected_contrasts_hisem_famsize_df, cohensd_hisem_famsize_df))
(losem_famsize_df <- bind_cols(selected_contrasts_losem_famsize_df, cohensd_losem_famsize_df))
(hisem_basefrq_df <- bind_cols(selected_contrasts_hisem_basefrq_df, cohensd_hisem_basefrq_df) )
(basefrq_df <- bind_cols(selected_contrasts_losem_basefrq_df, cohensd_losem_basefrq_df))
(sensitivity.familysize.basefreq_means <- as.data.frame(emmeans_obj$emmeans))
```
## Model Comparisons
...
reduced_model <- update(anova_model_1a,
                        . ~ . - family_size - Semantic_Sensitivity:family_size - family_size:base_freq - Semantic_Sensitivity:family_size:base_freq)
anova(anova_model_1a, reduced_model)

# Base Frequency
reduced_model <- update(anova_model_1a,
                        . ~ . - base_freq - Semantic_Sensitivity:base_freq - base_freq:family_size - Semantic_Sensitivity:base_freq:family_size)
anova(anova_model_1a, reduced_model)

# Family Size x Base Frequency
reduced_model_int <- update(anova_model_1a,
  . ~ . - family_size:base_freq - Semantic_Sensitivity:family_size:base_freq)
anova(anova_model_1a, reduced_model_int)

# Semantic Sensitivity x Family Size x Base Frequency
reduced_model_int <- update(anova_model_1a,
  . ~ .  - Semantic_Sensitivity:family_size:base_freq)
anova(anova_model_1a, reduced_model_int)
```
...
p4 <- sensitivity.familysize.basefreq_means|>
  ggplot(aes(x = family_size,
             y = emmean,
             fill = base_freq,
             colour = base_freq)) +
  facet_grid( ~ Semantic_Sensitivity) +
  geom_col(alpha = .4, position = position_dodge(.9)) +
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), 
                width = .15,
                position = position_dodge(.9)) +
  labs(y="Mean ERP amplitude (in microvolts)") +
  geom_text(aes(label = round(emmean, digits = 2)),
                colour = "black",size = 2.5, vjust = -7, position = position_dodge(.9)) +
  coord_cartesian(ylim = c(-1.7, .7)) +
  scale_color_custom() +
  scale_fill_custom() +
  labs(title = "Morphological Family Size x Base Frequency x Semantic Sensitivity") +
  theme( plot.title = element_text(size = 10, hjust = 0.5),
         legend.title = element_blank(),
         axis.title.x = element_blank(),
         axis.text.x = element_text(size = 8))
...
|                            Effect|      df|         F| p.value||eta-sqrd|  
|----------------------------------|--------|----------|--------|---------|
|   Semantic_Sensitivity:complexity| 1, 2121|  11.16 **|   <.001|5.23e-03 | 

*Interactions*

```{r}
# `Semantic_Sensitivity` x `Complexity` interaction
selected_contrasts<- c("High Semantic Complex - High Semantic Simple",
                       "Low Semantic Complex - Low Semantic Simple")

emmeans_obj <- emmeans(anova_model_1b, pairwise ~ Semantic_Sensitivity * complexity, adjust = "bonferroni", pbkrtest.limit = 6480)

# Get selected contrasts and convert the emmGrid object to a dataframe
contrasts_df <- as.data.frame(emmeans_obj$contrasts)
selected_contrasts_df <- as.data.frame(emmeans_obj$contrasts[contrasts_df$contrast %in% selected_contrasts, ])

cohensd_hisem <- as.data.frame(cohens_d(value ~ complexity, data = subset(n250_nonwords, Semantic_Sensitivity == "High Semantic")))
cohensd_losem <- as.data.frame(cohens_d(value ~ complexity,  data = subset(n250_nonwords, Semantic_Sensitivity == "Low Semantic")))

cohensd_df <- bind_rows(cohensd_hisem,cohensd_losem, .id = "sensitivity" )

(sensitivity.complexity_contrasts_df <- bind_cols(selected_contrasts_df,cohensd_df))
(sensitivity.complexity_means <- as.data.frame(emmeans_obj$emmeans))
```
...
# Reduced model: remove Semantic_Sensitivity:complexity interaction and all higher-order terms that include it
reduced_model <- update(anova_model_1b,
  . ~ . - Semantic_Sensitivity:complexity - Semantic_Sensitivity:family_size:complexity
)
# Model comparison
anova(anova_model_1b, reduced_model)
```

```{r,  echo=FALSE, fig.width=5, fig.height=3.5}
p3 <- sensitivity.complexity_means|>
  ggplot(aes(x = Semantic_Sensitivity,
             y = emmean,
             fill = complexity,
             colour = complexity)) +
  geom_col(alpha = .4, position = position_dodge(.9)) +
  ...
  scale_color_custom() +
  scale_fill_custom() +
  labs(title = "Semantic Sensitivity x Morphological Complexity") +
  theme( plot.title = element_text(size = 10, hjust = 0.5),
         legend.title = element_blank(),
         axis.title.x = element_blank(),
         axis.text.x = element_text(size = 8))
...
```{r}
p2 <- sensitivity.complexity_means |>
  ggplot(aes(x = Semantic_Sensitivity,
             y = emmean,
             fill = complexity,
             colour = complexity)) +
  ...
  scale_color_custom() +
  scale_fill_custom() +
  labs(title = "Semantic Sensitivity x Complexity") +
  theme( plot.title = element_text(size = 10, hjust = 0.5),
         legend.title = element_blank(),
         axis.title.x = element_blank(),
         axis.text.x = element_text(size = 8))
...
|             family_size:base_freq| 1, 2121| 23.02 ***|   <.001|0.01     |
| Sensitivity:family_size:base_freq| 1, 2121|   3.12 + |    .078|1.47e-03 |
...
```{r}
## `family_size` main effect
emmeans_obj <- emmeans(anova_model_2a, pairwise ~ family_size, adjust = "bonferroni", pbkrtest.limit = 6480)
(contrasts_df <- as.data.frame(emmeans_obj$contrasts))
cohensd_df <- as.data.frame(cohens_d(value ~ family_size, data = n400_words_b))
(family_size_contrasts_df <- bind_cols(contrasts_df,cohensd_df))
(family_size_means <- as.data.frame(emmeans_obj$emmeans))
...
cohensd_hi_basefrq <- as.data.frame(cohens_d(value ~ family_size,data = subset(n400_words_b, base_freq == "High Base Frequency")))
cohensd_lo_basefrq <- as.data.frame(cohens_d(value ~ family_size,data = subset(n400_words_b, base_freq == "Low Base Frequency")))
cohensd_lrg_fam <- as.data.frame(cohens_d(value ~ base_freq,data = subset(n400_words_b, family_size == "Large Family")))
cohensd_sml_fam <- as.data.frame(cohens_d(value ~ base_freq,data = subset(n400_words_b, family_size == "Small Family")))

cohensd_basefrq <- bind_rows(hi_basefrq = cohensd_hi_basefrq,lo_basefrq = cohensd_lo_basefrq,.id = "base_freq")
cohensd_famsize <- bind_rows(lrg_fam = cohensd_lrg_fam,sml_fam = cohensd_sml_fam,.id = "family_size")
...
emmeans_obj <- emmeans(anova_model_2a,  pairwise ~ Semantic_Sensitivity * family_size * base_freq, adjust = "bonferroni",  pbkrtest.limit = 6480)
...
cohensd_1 <- as.data.frame(cohens_d(value ~ base_freq, data = subset(n400_words_b, Semantic_Sensitivity == "High Semantic" & family_size == "Large Family")))
cohensd_2 <- as.data.frame(cohens_d(value ~ base_freq, data = subset(n400_words_b, Semantic_Sensitivity == "High Semantic" & family_size == "Small Family")))

cohensd_3 <- as.data.frame(cohens_d(value ~ base_freq,  data = subset(n400_words_b, Semantic_Sensitivity == "Low Semantic" & family_size == "Large Family")))
cohensd_4 <- as.data.frame(cohens_d(value ~ base_freq, data = subset(n400_words_b, Semantic_Sensitivity == "Low Semantic" & family_size == "Small Family")))

cohensd_5 <- as.data.frame(cohens_d(value ~ family_size,  data = subset(n400_words_b, Semantic_Sensitivity == "High Semantic" & base_freq == "High Base Frequency")))
cohensd_6 <- as.data.frame(cohens_d(value ~ family_size,  data = subset(n400_words_b, Semantic_Sensitivity == "High Semantic" & base_freq == "Low Base Frequency")))

cohensd_7 <- as.data.frame(cohens_d(value ~ family_size,  data = subset(n400_words_b, Semantic_Sensitivity == "Low Semantic" & base_freq == "High Base Frequency")))
cohensd_8 <- as.data.frame(cohens_d(value ~ family_size, data = subset(n400_words_b, Semantic_Sensitivity == "Low Semantic" & base_freq == "Low Base Frequency")))
...
```{r}
# Family Size
reduced_model <- update(anova_model_2a,
                        . ~ . - family_size - Semantic_Sensitivity:family_size - family_size:base_freq - Semantic_Sensitivity:family_size:base_freq)
anova(anova_model_1a, reduced_model)

# Family Size x Base Frequency
reduced_model_int <- update(anova_model_2a,
  . ~ . - family_size:base_freq - Semantic_Sensitivity:family_size:base_freq)
anova(anova_model_1a, reduced_model_int)
...
p3 <- famsize.basefreq_means|>
  ...
  scale_color_custom() +
  scale_fill_custom() +
  labs(title = "Base Frequency x Morphological Family Size") +
  theme( plot.title = element_text(size = 10, hjust = 0.5),
         legend.title = element_blank(),
         axis.title.x = element_blank(),
         axis.text.x = element_text(size = 8))

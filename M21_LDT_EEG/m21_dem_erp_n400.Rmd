...
# Trim data to 2SD of each subject's mean
```{r}
n400_1_nw_trm <- n400_1_nonwords %>%
  group_by(SubjID) %>%
  filter(abs(value - mean(value)) <= 3.5 * sd(value)) 
```
...
Plot interaction `lang_type_ortho | anteriority`
...
Plot interaction `complexity | lang_type_semantic * anteriority`
```{r, fig.width=5, fig.height=3}
# plot raw scores

...
# estimated marginal means for each level of complexity, adjusted for zLogBF, zLogFS, OS, and LP
(emm_complexity <- emmeans(m_rt_nw,  ~ complexity, lmer.df = "satterthwaite"))

# Because your outcome is logRT, these means are on the log reaction-time scale. 
# To back-transform them to RTs
emm_complexity <- emmeans(m_rt_nw, ~ complexity, tran = "log", lmer.df = "satterthwaite")
summary(emm_complexity, type = "response")

# Step 1: Get emmeans on log scale with Satterthwaite df
emm_log <- emmeans(m_rt_nw, ~ complexity, lmer.df = "satterthwaite")
summary(emm_log)  # check df are no longer Inf

# Step 2: Back-transform manually
emm_summary <- summary(emm_log)
emm_summary$mean_ms  <- exp(emm_summary$emmean)
emm_summary$lower_ms <- exp(emm_summary$asymp.LCL)
emm_summary$upper_ms <- exp(emm_summary$asymp.UCL)
emm_summary


#To compare the two complexity levels:
pairs(emm_complexity)

# Back-transformed contrast (ratio rather than difference on log scale)
pairs(emmeans(m_rt_nw, ~ complexity, tran = "log"), 
      type = "response")

# on the back-transformed scale the contrast becomes a ratio rather than a difference in ms — 
# complex/simple rather than complex − simple. To get the difference in ms directly, compute it from the 
# # back-transformed means

emm_summary <- summary(emm_complexity)
emm_summary$emmean_ms <- exp(emm_summary$emmean)
emm_summary$lower_ms  <- exp(emm_summary$asymp.LCL)
emm_summary$upper_ms  <- exp(emm_summary$asymp.UCL)
emm_summary

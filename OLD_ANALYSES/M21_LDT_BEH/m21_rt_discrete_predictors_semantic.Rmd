...
```{r remove_NA_words, results = "hide"}
...
```{r get_condition_counts_words, echo=FALSE, results = "hide"}
rt_words_cmpl %>%
  summarise(
...
|                        Effect|       df |      F |p.value|
|------------------------------|----------|--------|-------|
|                Base_Frequency| 1, 93.73 |10.08 **|   .002|
|                   Family_Size| 1, 92.52 | 9.00 **|   .003|
...
emmeans(anova_model_words, ~ Semantic_Sensitivity)
```

### Plots


```{r words_main_effects_plots, echo = FALSE, fig.height=3}
...
```{r remove_NA_nwords, results = "hide"}
# Specify only the variables used in the model
...
```{r get_condition_counts_nwords, echo=FALSE, results = "hide"}
rt_nwords_cmpl %>%
...
emmeans(anova_model_nwords_fs, ~ Semantic_Sensitivity)
```
...
```{r main_effects_plots_nwords_fs, echo = FALSE, fig.height=3, fig.width=3.5}
...
|                                     Effect|          df |        F |p.value |
|-------------------------------------------|-------------|----------|--------|
|Complexity:Family_Size:Semantic_Sensitivity|  1,  198.28 |  4.67   *|    .032|
...
```{r nwords_fs_interaction_contrasts}
...
```{r nwords_fs_interaction_plots, fig.height=3, fig.width=8}
p4 <- emmip(anova_model_nwords_fs, Complexity ~ Family_Size | Semantic_Sensitivity) + my_style
p5 <- emmip(anova_model_nwords_fs, Family_Size ~ Complexity | Semantic_Sensitivity) + my_style
...
|                     Effect|          df |        F |p.value |
|---------------------------|-------------|----------|--------|
|                 Complexity|  1,   61.44 | 88.55 ***|   <.001|
|             Base_Frequency|  1,   91.73 | 11.22  **|    .001|
...
emmeans(anova_model_nwords_bf, ~ Semantic_Sensitivity)
```
...
|                     Effect|          df |        F |p.value |
|---------------------------|-------------|----------|--------|
|  Complexity:Base_Frequency|  1,  545.66 |  3.14   *|     077|
...
A marginal Complexity × Base Frequency interaction (p = .048) suggests that the complexity cost was smaller for nonwords derived from low-frequency bases.
...
```{r nwords_bf_main_effects_plots, echo = FALSE, fig.height=3}
#### Main Effect of Complexity
# Compute means for each level of Complexity
(emm_nw_bf_cmp_df <- as.data.frame(emmeans(anova_model_nwords_bf, ~  Complexity )))

p6<- emm_nw_bf_cmp_df |> ggplot(aes(y = emmean, x = Complexity, 
                                   colour = Complexity, 
                                   fill = Complexity)) +
...
p7<- emm_nw_bf_bf_df |> ggplot(aes(y = emmean, x = Base_Frequency, 
...
(emm_nw_bf_bf_df <- as.data.frame(emmeans(anova_model_nwords_bf, ~  Semantic_Sensitivity )))

p8 <- emm_nw_bf_bf_df |> ggplot(aes(y = emmean, x = Semantic_Sensitivity, 
...
plot_grid(p6, p7, p8, ncol = 3, labels = "AUTO")
```

###  Interaction Plots

```{r nwords_bf_interaction_plots, fig.height=3}
p9 <- emmip(anova_model_nwords_bf, Complexity ~ Base_Frequency) + my_style
p10 <- emmip(anova_model_nwords_bf, Base_Frequency ~ Complexity) + my_style

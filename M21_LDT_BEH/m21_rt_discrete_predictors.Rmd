...
```{r}
DIR <- "csv_files"
rt_1 <- read_csv(file.path(DIR, "rt_data_hc.csv"))
...
rt_nwords_frq <-  left_join(rt_nwords, frq_nw, join_by(target==word)) |>
  select(-complexity.x) |>
  rename(Complexity = complexity.y)  # remove duplicate columns and rename
...
```{r}
# Extract lmer models
m1 <- anova_model_words$full_model      # (1 ¦ SubjID)
m2 <- anova_model_words_2$full_model    # (1 ¦ SubjID) + (1 ¦ STRING)
...
# compute the intra-class correlation (ICC) to see the relative contributions
VarCorr(m2)
performance::icc(m2)

# formally test whether adding each random effect improves fit
ranova(m2)
```
...
|                        Effect|       df |      F |p.value|
|------------------------------|----------|--------|-------|
|                Base_Frequency| 1, 92.48 |10.10 **|   .002|
|                   Family_Size| 1, 92.48 | 9.41 **|   .003|
...
mean_df <- rt_words_cmpl %>%
  group_by(Base_Frequency) %>%
  summarise(mean_rt = mean(response_time, na.rm = TRUE))

rt_words_cmpl |> ggplot(aes(x = response_time, colour = Base_Frequency, fill = Base_Frequency)) +
  geom_density( alpha = .4) +
  geom_vline(data = mean_df,
             aes(xintercept = mean_rt, colour = Base_Frequency),
...
rt_words_cmpl |> ggplot(aes(x = response_time, colour = Family_Size, fill = Family_Size)) +
  geom_density( alpha = .4) +
  geom_vline(data = mean_df,
             aes(xintercept = mean_rt, colour = Family_Size),
             linetype = "dashed", size = 1) +
  labs(title = "Words") +
  scale_color_custom() +
  scale_fill_custom() 
...
incomplete_cases_nwords <- rt_nwords[!complete.cases(rt_nwords_frq[, model_vars]), ]
...
```{r}
anova_model_nwords <- mixed(
  response_time ~ Complexity * Base_Frequency * Family_Size * Orthographic_Sensitivity +
    (1 | SubjID) +
    (1 | target),
  data = rt_nwords_cmpl,
  method = "S")
...
m2 <- anova_model_nwords$full_model    # Extract the lmer model

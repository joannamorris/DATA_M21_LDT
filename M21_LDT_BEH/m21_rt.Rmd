...
```{r}
DIR <- "csv_files"
rt_1 <- read_csv(file.path(DIR, "rt_data_chrt1.csv"))
...
rt_words_frq$BF_MedianSplit <- ifelse(rt_words_frq$Log10BF <= median(rt_words_frq$Log10BF, na.rm = TRUE),"Low", "High")
rt_words_frq$BF_MedianSplit <- factor(rt_words_frq$BF_MedianSplit, levels = c("Low", "High"))

rt_words_frq$FS_Split <- factor(rt_words_frq$FS_MedianSplit)
rt_words_frq$FS_Split <- recode(x, a = "alpha", b = "beta")
...
p2 <- rt_nwords_1 |> ggplot(aes(x = response_time)) +
  geom_density(colour = "#E31A1C", fill = "#E31A1C", alpha = .4)
plot_grid(p1, p2, ncol = 2)

# InvRT  boxplot
p1 <- rt_words_1 |> ggplot(aes(x = SubjID, y = InvRT)) +
  geom_boxplot(colour = "#1F78B4", fill = "#1F78B4", alpha = .4)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
p2 <- rt_nwords_1 |> ggplot(aes(x = SubjID, y = InvRT)) +
  geom_boxplot(colour = "#E31A1C", fill = "#E31A1C", alpha = .4)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
plot_grid(p1, p2, ncol = 2)

# InvRT density plot
p1 <- rt_words_1 |> ggplot(aes(x = InvRT)) +
  geom_density(colour = "#1F78B4", fill = "#1F78B4", alpha = .4) 

p2 <- rt_nwords_1 |> ggplot(aes(x = InvRT)) +
  geom_density(colour = "#E31A1C", fill = "#E31A1C", alpha = .4) 
plot_grid(p1, p2, ncol = 2)
```
...
# Raw FS
ggplot(rt_words_1, aes(x = FS)) +
  geom_density(colour = "#B2DF8A", fill = "#B2DF8A", alpha = .4) +
  labs(title = "Distribution of Family Size")
# Log10 FS
ggplot(rt_words_1, aes(x = Log10FS)) +
  geom_density(colour = "#B2DF8A", fill = "#B2DF8A", alpha = .4) +
  labs(title = "Distribution of Family Size")
...
|                     Effect|         df |       F |p.value |
|---------------------------|------------|---------|--------|
|                Log10BF_std| 1, 5792.69 |41.88 ***|   <.001|
|                   FS_Split| 1, 5792.31 |23.20 ***|   <.001|
|Log10BF_std:lang_type_ortho| 1, 5792.69 |   4.76 *|    .029|
